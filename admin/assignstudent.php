<?php
   ...
   if (!empty($user) && !empty($subject) && !empty($section) && !empty($faculty)) 
        {
      //Insert Query//
    foreach($subject as $sub){
    $query = "INSERT INTO assign_tbl(student_id,subject_id,section_id,faculty_id) VALUES('$user','$sub','$section','$faculty')";
      
      $result = mysqli_query($con, $query);
        
        //Redirect the user in login modulee if all the information needed are completed//
        $_SESSION['success_message'] = "Student is succesfully assigned!";
...
<html lang="en">
  <head>
    ...
    <title>BORA - SIS | Admin - Assign Students</title>
...
<body id="page-top">
 ...
 <div class="container-fluid">
                    ...
                    <!-- Page Heading -->
                    <h1 class="h3 mb-2 text-gray-800">Assign Students</h1>
...
?>
  ...
  ?>
   ...
   <select name="subject[]" class="form-control" multiple>
    <option selected disabled></option>
    <?php 
    foreach ($options as $option) {
  ?>
  ...
  <div class="form-group">
   ...
   <select name="section" class="form-control">
    ...
    foreach ($options as $option) {
  ?>
    <option value="<?php echo $option['sec_id']?>"><?php echo   $option['section_name']; ?> </option>
    <?php 
    }
   ?>
  </select>
  ...
  <?php 
    ...
    ?>     
    <label for="faculty">Adviser</label>
